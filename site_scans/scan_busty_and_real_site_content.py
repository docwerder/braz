...
for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/60/busty-real/sortby/views/page/" + str(lf)
    ...
    db_bar = pd.concat([db_bar, db_all_tmp])

db_bar = db_bar.reset_index()
del db_bar['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_busty_and_real_site_content.csv')
print('save_file: ', save_file)

db_bar_sliced = db_bar.dropna()
print(tabulate(db_bar_sliced, headers='keys', tablefmt='psql'))

db_bar_sliced.to_csv(save_file)
