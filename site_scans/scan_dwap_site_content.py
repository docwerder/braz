...
ps2_db = {}
title_db = {}
db_dwap = pd.DataFrame()

for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/87/day-with-a-pornstar/sortby/views/page/" + str(lf)
    ...
    db_dwap = pd.concat([db_dwap, db_all_tmp])

db_dwap = db_dwap.reset_index()
del db_dwap['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_dwap_site_content.csv')
print('save_file: ', save_file)

db_dwap_sliced = db_dwap.dropna()
print(tabulate(db_dwap_sliced, headers='keys', tablefmt='psql'))

db_dwap_sliced.to_csv(save_file)
