...
ps2_db = {}
title_db = {}
db_btis = pd.DataFrame()

for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/83/big-tits-in-sports/sortby/views/page/" + str(lf)
    ...
    db_btis = pd.concat([db_btis, db_all_tmp])

db_btis = db_btis.reset_index()
del db_btis['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_btis_site_content.csv')
print('save_file: ', save_file)

db_btis_sliced = db_btis.dropna()
print(tabulate(db_btis_sliced, headers='keys', tablefmt='psql'))

db_btis_sliced.to_csv(save_file)
