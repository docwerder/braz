...
for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/92/zz-series/sortby/views/page/" + str(lf)
    ...
    db_zzs = pd.concat([db_zzs, db_all_tmp])

db_zzs = db_zzs.reset_index()
del db_zzs['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_zzseries_site_content.csv')
print('save_file: ', save_file)

db_zzs_sliced = db_zzs.dropna()
print(tabulate(db_zzs_sliced, headers='keys', tablefmt='psql'))

db_zzs_sliced.to_csv(save_file)
