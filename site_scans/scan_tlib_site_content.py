...
ps2_db = {}
title_db = {}
db_tlib = pd.DataFrame()

for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/80/teens-like-it-big/sortby/views/page/" + str(lf)
    ...
    db_tlib = pd.concat([db_tlib, db_all_tmp])

db_tlib = db_tlib.reset_index()
del db_tlib['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_tlib_site_content.csv')
print('save_file: ', save_file)

db_tlib_sliced = db_tlib.dropna()
print(tabulate(db_tlib_sliced, headers='keys', tablefmt='psql'))

db_tlib_sliced.to_csv(save_file)
