...
ps2_db = {}
title_db = {}
db_cfnm = pd.DataFrame()

for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/98/cfnm/sortby/views/page/" + str(lf)
    ...
    db_cfnm = pd.concat([db_cfnm, db_all_tmp])

db_cfnm = db_cfnm.reset_index()
del db_cfnm['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_cfnm_site_content.csv')
print('save_file: ', save_file)

db_cfnm_sliced = db_cfnm.dropna()
print(tabulate(db_cfnm_sliced, headers='keys', tablefmt='psql'))

db_cfnm_sliced.to_csv(save_file)
