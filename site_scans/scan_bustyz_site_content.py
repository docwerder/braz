...
ps2_db = {}
title_db = {}
db_bustyz = pd.DataFrame()

for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/63/bustyz/sortby/views/page/" + str(lf)
    ...
    db_bustyz = pd.concat([db_bustyz, db_all_tmp])

db_bustyz = db_bustyz.reset_index()
del db_bustyz['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_bustyz_site_content.csv')
print('save_file: ', save_file)

db_bustyz_sliced = db_bustyz.dropna()
print(tabulate(db_bustyz_sliced, headers='keys', tablefmt='psql'))

db_bustyz_sliced.to_csv(save_file)
