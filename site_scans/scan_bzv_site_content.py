...
for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/84/brazzers-vault/sortby/views/page/" + str(lf)
    ...
    db_bzv = pd.concat([db_bzv, db_all_tmp])

db_bzv = db_bzv.reset_index()
del db_bzv['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_bzv_site_content.csv')
print('save_file: ', save_file)

db_bzv_sliced = db_bzv.dropna()
print(tabulate(db_bzv_sliced, headers='keys', tablefmt='psql'))

db_bzv_sliced.to_csv(save_file)
