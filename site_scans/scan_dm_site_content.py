...
for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/94/dirty-masseur/sortby/views/page/" + str(lf)
    ...
    db_dm = pd.concat([db_dm, db_all_tmp])

db_dm = db_dm.reset_index()
del db_dm['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_dm_site_content.csv')
print('save_file: ', save_file)

db_dm_sliced = db_dm.dropna()
print(tabulate(db_dm_sliced, headers='keys', tablefmt='psql'))

db_dm_sliced.to_csv(save_file)
