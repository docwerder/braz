...
for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/99/moms-in-control/sortby/views/page/" + str(lf)
    ...
    db_mic = pd.concat([db_mic, db_all_tmp])

db_mic = db_mic.reset_index()
del db_mic['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_mic_site_content.csv')
print('save_file: ', save_file)

db_mic_sliced = db_mic.dropna()
print(tabulate(db_mic_sliced, headers='keys', tablefmt='psql'))

db_mic_sliced.to_csv(save_file)
