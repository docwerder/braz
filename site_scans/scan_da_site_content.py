...
for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/62/doctor-adventures/sortby/views/page/" + str(lf)
    ...
    db_da = pd.concat([db_da, db_all_tmp])

db_da = db_da.reset_index()
del db_da['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_da_site_content.csv')
print('save_file: ', save_file)

db_da_sliced = db_da.dropna()
print(tabulate(db_da_sliced, headers='keys', tablefmt='psql'))

db_da_sliced.to_csv(save_file)
