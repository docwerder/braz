...
for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/96/brazzers-exxtra/sortby/views/page/" + str(lf)
    ...
    db_bbe = pd.concat([db_bbe, db_all_tmp])

db_bbe = db_bbe.reset_index()
del db_bbe['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_bbe_site_content.csv')
print('save_file: ', save_file)

db_bbe_sliced = db_bbe.dropna()
print(tabulate(db_bbe_sliced, headers='keys', tablefmt='psql'))

db_bbe_sliced.to_csv(save_file)
