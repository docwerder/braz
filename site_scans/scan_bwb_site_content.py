...
for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/65/big-wet-butts/sortby/views/page/" + str(lf)
    ...
    db_bwb = pd.concat([db_bwb, db_all_tmp])

db_bwb = db_bwb.reset_index()
del db_bwb['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_bwb_site_content.csv')
print('save_file: ', save_file)

db_bwb_sliced = db_bwb.dropna()
print(tabulate(db_bwb_sliced, headers='keys', tablefmt='psql'))

db_bwb_sliced.to_csv(save_file)
