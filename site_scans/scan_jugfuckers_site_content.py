...
for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/69/jug-fuckers/sortby/views/page/" + str(lf)
    ...
    db_juf = pd.concat([db_juf, db_all_tmp])

db_juf = db_juf.reset_index()
del db_juf['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_jugfuckers_site_content.csv')
print('save_file: ', save_file)

db_juf_sliced = db_juf.dropna()
print(tabulate(db_juf_sliced, headers='keys', tablefmt='psql'))

db_juf_sliced.to_csv(save_file)
