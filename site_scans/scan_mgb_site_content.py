...
number = range(1,25)  # 25  # Note: THe last video not within the dataframe!!!
...
for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/67/mommy-got-boobs/sortby/views/page/" + str(lf)
    ...
    db_mgb = pd.concat([db_mgb, db_all_tmp])

db_mgb = db_mgb.reset_index()
del db_mgb['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_mgb_site_content.csv')
print('save_file: ', save_file)

db_mgb_sliced = db_mgb.dropna()
print(tabulate(db_mgb_sliced, headers='keys', tablefmt='psql'))

db_mgb_sliced.to_csv(save_file)
