...
ps2_db = {}
title_db = {}
db_hcba = pd.DataFrame()

for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/64/hot-chicks-big-asses/sortby/views/page/" + str(lf)
    ...
    db_hcba = pd.concat([db_hcba, db_all_tmp])

db_hcba = db_hcba.reset_index()
del db_hcba['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_hot_chicks_big_asses_site_content.csv')
print('save_file: ', save_file)

db_hcba_sliced = db_hcba.dropna()
print(tabulate(db_hcba_sliced, headers='keys', tablefmt='psql'))

db_hcba_sliced.to_csv(save_file)
