...
ps2_db = {}
title_db = {}
db_mlib = pd.DataFrame()

for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/78/milfs-like-it-big/sortby/views/page/" + str(lf)
    ...
    db_mlib = pd.concat([db_mlib, db_all_tmp])

db_mlib = db_mlib.reset_index()
del db_mlib['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_mlib_site_content.csv')
print('save_file: ', save_file)

db_mlib_sliced = db_mlib.dropna()
print(tabulate(db_mlib_sliced, headers='keys', tablefmt='psql'))

db_mlib_sliced.to_csv(save_file)
