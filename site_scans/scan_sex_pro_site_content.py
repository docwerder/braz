...
for lf in number:
    actual_site = "https://www.brazzers.com/videos/site/76/sex-pro-adventures/sortby/views/page/" + str(lf)
    ...
    db_sp = pd.concat([db_sp, db_all_tmp])

db_sp = db_sp.reset_index()
del db_sp['index']

root = os.getcwd()
save_path = os.path.join(root, 'scanned_site_content')
save_file = os.path.join(save_path, 'db_sex_pro_site_content.csv')
print('save_file: ', save_file)

db_sp_sliced = db_sp.dropna()
print(tabulate(db_sp_sliced, headers='keys', tablefmt='psql'))

db_sp_sliced.to_csv(save_file)
